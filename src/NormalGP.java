...
public class NormalGP  extends Model
{
    ...
    public NormalGP(double t0, double t1, double h, Vector x0)
    {
        super(t0, t1, h);
        double po0 = Math.sqrt(Math.pow(x0.data[0], 2) + Math.pow(x0.data[1], 2)
        +Math.pow(x0.data[2], 2));
        double fi = Math.atan2(x0.data[2] , Math.sqrt(Math.pow(x0.data[0], 2) + Math.pow(x0.data[1], 2)));
